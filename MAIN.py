    ...
    for table in doc.tables:
        for row in table.rows:
            if row.cells[0].text == id_user:
                ...
                information = [id_user, username, password, money, match, win, lose]
                return information

def update_money(id_user, moremoney):
    docx_filename = "Accounts.docx"
    doc = Document(docx_filename)
    for table in doc.tables:
        for row in table.rows:
...
class Game:
    def __init__(self):
        ...
        #User's variable
        self.player_name = get_information(ID_Login[0])[1]
        self.money = get_information(ID_Login[0])[3]
        self.bet = 200
